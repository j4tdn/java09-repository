package format;

import java.math.BigDecimal;
import java.math.RoundingMode;
import java.text.DecimalFormat;
import java.text.NumberFormat;
import java.util.Locale;
import java.util.Random;

public class Ex02 {
	public static void main(String[] args) {
		//Math
		System.out.println("round: " + Math.round(5.7));
		System.out.println("floor: " + Math.floor(5.7));
		// random
		Random rd = new Random();
		// 0 - n-1
		System.out.println("rand 01: " + rd.nextInt(10));
		
		//a - b
		System.out.println("a - b: " + (5 + rd.nextInt(16)));
		
		//format text: number, currency
		// number format. decimal format
		float number = 191919.789f;
//		decimal: lưu số dưới 10 chỉ số
		DecimalFormat formatter = new DecimalFormat("#,###.####");
		System.out.println("number: " + formatter.format(number));
		
//		BigDecimal
//		float bigNumber = 123456.78901f;
		BigDecimal b1 = BigDecimal.valueOf(123456.78901).setScale(5,RoundingMode.HALF_UP);
		System.out.println("b1: " + b1);
		
		//number format
		int number1 = 1234;
		Locale locale = new Locale("vi", "VN");
		NumberFormat nf = NumberFormat.getCurrencyInstance(locale);
		System.out.println(nf.format(number1));
	}
}
