...
public class Ex04 {
	 ...
	 * hiện tại có bao nhiêu ngày. 3. In ngày cuối cùng của tháng hiện tại
	 * (dd/MM/yyyy). 4. Xem ngày đầu tiên và cuối cùng của tuần hiện tại. 5. Xem
	 * ngày hiện tại đang ở tuần thứ mấy trong năm. 6. Thêm 8 tiếng vào thời gian
	 * hiện tại và in ra. 7. Kiểm tra khoảng cách giữa 2 mốc thời gian là bao nhiêu
	 * năm, tháng. 8. Kiểm tra sau 20 ngày nữa là ngày nào, thứ mấy. 9. Kiểm tra số
	 * tháng còn lại trong năm từ thời điểm hiện tại. 10.Tính tuổi của bạn. Bao
	 * nhiêu năm, tháng, ngày.
	 */
	...
	public static void p7() {
		...
		System.out.println("years: " + differenceYear + ", months: " + differenceMonth);
	}
	...
	private static void p10() {
		...
		Calendar birthday = Calendar.getInstance();
		birthday.set(YEAR, startYear);
		birthday.set(MONTH, startMonth - 1);
		birthday.set(DAY_OF_MONTH, startDay);
		Calendar now = Calendar.getInstance();
		int endYear = now.get(YEAR);
		int endMonth = now.get(MONTH);
		...
		Months = monthsBetween(birthday, now);
		if (endDay >= startDay) {
			...
			tmp.add(MONTH, -1);
			Days = tmp.getActualMaximum(DAY_OF_MONTH) - startDay + endDay;
			if (endMonth == startMonth) {
